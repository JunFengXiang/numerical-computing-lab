...
其内积定义为 $(f, g) = \sum_{k=1}^{n} f_{k} g_{k}$; 向量 $f \in \mathbf{R}^{n}$ 的模(范数)定义为
$$\| f \|_{2} = \left( \sum_{k=1}^{n} f_{k}^{2} \right)^{\frac{1}{2}},$$
将它推广到任何内积空间中就有下面的定义.

定义 3.6 $f(x) \in C[a, b]$, 量
$$\| f \|_{2} = \sqrt{\int_{a}^{b} \rho(x) f^{2}(x) \mathrm{d}x} = \sqrt{(f, f)}$$
(3.3.4)
...
$1^{\circ} |(f, g)| \leqslant \| f \|_{2} \| g \|_{2}$ (Cauchy-Schwarz 不等式); (3.3.5)
$2^{\circ} \| f + g \|_{2} \leqslant \| f \|_{2} + \| g \|_{2}$ (三角不等式); (3.3.6)
