...
当权函数 $\rho(x) = \frac{1}{\sqrt{1-x^2}}$, 区间为 $[-1, 1]$ 时, 由序列 $\{1, x, x^2, \cdots, x^n, \cdots\}$ 正交得到的正交多项式就是 Chebyshev多项式, 它可表示为
$T_n(x) = \cos(n \arccos x), \quad |x| \leqslant 1.$ (3.4.6)
...
$T_{n+1}(x) = 2xT_n(x) - T_{n-1}(x) \quad (n = 1, 2, \cdots).$ (3.4.7)
