...
> 校注（agent 补充，CH10-E001）：本页化简式及其后两个 $N/2$-WT 的第二式，原图均写作 $H_{N/2}(N/2+i,j)$，上文已照录。按前页定理 10.1 和本章对 $N/2$ 阶矩阵的索引范围，这里的行指标 $N/2+i$ 超出 $0,\ldots,N/2-1$；由 $H_N(N/2+i,j)=H_{N/2}(i,j)$，两处应为 $H_{N/2}(i,j)$。这是原书下标疑误，不是 OCR 错误。[原式放大裁片](../assets/erratum-p271-indices.png)。此外，本页“定理 1”照录，所用递推关系对应前页“定理 10.1”。
