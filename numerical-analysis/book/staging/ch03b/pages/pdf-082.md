...
> 校注（agent 补充）：式 (3.6.11) 第一行原书印为拉丁字母 $a_{k+1}$，式 (3.6.10) 则为希腊字母 $\alpha_{k+1}$，已分别保留。该式末的共同范围原书为 $k=0,1,\cdots,n-1$；若直接用于第二行 $\beta_k$，$k=0$ 会涉及未定义的 $P_{-1}$。依式 (3.6.10)，实际递推使用的是 $\beta_1,\ldots,\beta_{n-1}$。见 [原式裁切](../assets/p082-recursion-coefficients.png)。
