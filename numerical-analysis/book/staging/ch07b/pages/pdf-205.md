...
> 校注（agent 补充，原书疑误）：本页定理 7.18 证明原文为 $Bx_0=x_0$，以及 $(I\pm B)^{-1}=I\mp B(I-B)^{-1}$，均按印刷内容保留。若同时处理 $I+B$ 和 $I-B$ 两种情形，前式应为 $Bx_0=\mp x_0$，后式右端的逆矩阵应为 $(I\pm B)^{-1}$；原文写法只与减号分支一致。前式符号不影响随后所取范数等于 1 的结论。[原式局部图](../assets/205-inverse-signs.png)。
