...
> 校注（agent 补充，原书疑误）：向后误差分析段中印刷方程为 $(A+\delta A)x=b$，而紧邻文字明确指计算解 $\overline x$ 是其精确解。此处保留原式；按本段符号约定，应写 $(A+\delta A)\overline x=b$。[原文局部图](../assets/210-backward-error.png)。
