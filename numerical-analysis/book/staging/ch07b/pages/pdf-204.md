...
> 校注（agent 补充，原书疑误）：复矩阵 2-范数公式的 Rayleigh 商外，原图没有平方根或 $1/2$ 次幂，此处照录。该最大商等于 $\lambda_{\max}(A^HA)=\|A\|_2^2$；要与两侧 $\|A\|_2$ 和 $\sqrt{\lambda_{\max}(A^HA)}$ 一致，中间项需开平方。[原式局部图](../assets/204-complex-norm.png)。
