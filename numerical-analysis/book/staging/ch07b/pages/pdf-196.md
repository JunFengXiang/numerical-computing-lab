...
> 校注（agent 补充，原书疑误）：式（7.4.8）步 4 原图确印 $b_i$，这里忠实保留。由本页所列 $L^{\mathrm T}x=y$，回代右端应使用 $y_i$；否则与其前方三角方程组不一致。[原式局部图](../assets/196-step4.png)。
>
> 校注（agent 补充，原书疑误）：$LDL^{\mathrm T}$ 展开中最右矩阵次末行末项原图印作 $l_{n-1,n-1}$，这里保留原文；按左边 $L$ 的转置，该位置应为 $l_{n,n-1}$。[原矩阵局部图](../assets/196-ldlt.png)。
